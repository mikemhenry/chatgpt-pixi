...
2. GitHub verifies that the downloaded tar.gz archive and its `.sha256` sidecar belong to that release.
3. The archive digest matches Pixi's official per-archive `.tar.gz.sha256` sidecar.
4. `gh attestation verify` validates the SLSA provenance for the exact release archive and pinned release tag.
5. The archive is extracted and the resulting raw Pixi binary must match the digest committed in `upstream/pixi.sha256`.
